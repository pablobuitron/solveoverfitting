...
from ..data.splits import random_splits_generator, dataset_split, field_splits_generator, simple_splits_generator, \
    temporal_splits_generator, temporal_tidy_splits_generator
...
from ..utils.metrics_csv_wrapper import TrainingMetricsWrapper
from ..insights import plotting
...
SPLITS_GENERATOR_SIMPLE = 'simple'
SPLITS_GENERATOR_TEMPORAL = 'temporal'
SPLITS_GENERATOR_TEMPORAL_TIDY = 'temporal_tidy'
...
class ExperimentManager:
    ...
    def new_experiment(self, split_generator: str, pre_generate_splits: bool = False):
        ...
        exp.splits_generator = splits_gen
        exp.persist()
        self.experiment = exp
    ...
    def load(self, experiment_path: pathlib.Path):
        """Load a specific experiment"""
        self.experiment = Experiment.load(experiment_path)

    def train_experiment(self, training_configuration_override: dict = None):

        self._load_configuration_for_training(training_configuration_override)
        ...
        trainer = train_lstm.Trainer(training_config=self.experiment.training_config,
                                     model_architecture_config=self.experiment.model_architecture_config)
        trainer.train(dataset[dataset_constants.TRAIN_SPLIT_NAME],
                      dataset[dataset_constants.VALIDATION_SPLIT_NAME],
                      output_dir=self.experiment.dir_structure.new_training_res_version())

        self.experiment = Experiment.load(self.experiment.dir_structure.experiment_root_dir)
        self.plot_experiment_data()

    def plot_experiment_data(self, load_latest: bool = False):
        if load_latest:
            self.load_latest()

        training_metrics = self.experiment.training_metrics
        training_metrics_wrapper = TrainingMetricsWrapper(training_metrics)
        trends_plotter = plotting.TrendsPlotter()

        loss_trends = []
        loss_trends_labels = []

        training_loss_trend = training_metrics_wrapper.train_loss_trend()
        if training_loss_trend is not None:
            loss_trends.append(training_loss_trend)
            loss_trends_labels.append('Training Loss')
        else:
            logger.error("No training loss trend found. It won't be plotted.")

        validation_loss_trend = training_metrics_wrapper.val_loss_trend()
        if validation_loss_trend is not None:
            loss_trends.append(validation_loss_trend)
            loss_trends_labels.append('Validation Loss')
        else:
            logger.error("No validation loss trend found. It won't be plotted.")

        loss_trends_figure = trends_plotter.generate_figure(loss_trends, loss_trends_labels)
        loss_trends_output_path = (self.experiment.dir_structure.train_res_latest_version_dir / 'loss_trends.png')
        loss_trends_figure.savefig(loss_trends_output_path)

        rmse_trends = []
        rmse_trends_labels = []

        training_rmse = training_metrics_wrapper.train_rmse_trend()
        if training_rmse is not None:
            rmse_trends.append(training_rmse)
            rmse_trends_labels.append('Training RMSE')
        else:
            logger.error("No training rmse trend found. It won't be plotted.")

        validation_rmse = training_metrics_wrapper.val_rmse_trend()
        if validation_rmse is not None:
            rmse_trends.append(validation_rmse)
            rmse_trends_labels.append('Validation RMSE')
        else:
            logger.error("No validation rmse trend found. It won't be plotted.")

        rmse_trends_figure = trends_plotter.generate_figure(rmse_trends, rmse_trends_labels)
        rmse_trends_output_path = (self.experiment.dir_structure.train_res_latest_version_dir / 'rmse_trends.png')
        rmse_trends_figure.savefig(rmse_trends_output_path)

    def _init_splits_generator(self, generator_name: str) -> splits_generator.SplitsGenerator:
        ...
        elif generator_name == SPLITS_GENERATOR_FIELD_AND_YEAR:
            return field_splits_generator.FieldSplitsGenerator()
        elif generator_name == SPLITS_GENERATOR_SIMPLE:
            return simple_splits_generator.SimpleSplitsGenerator()
        elif generator_name == SPLITS_GENERATOR_TEMPORAL:
            return temporal_splits_generator.TemporalSplitsGenerator(self.dataset_root_dir)
        elif generator_name == SPLITS_GENERATOR_TEMPORAL_TIDY:
            return temporal_tidy_splits_generator.TemporalTidySplitGenerator()
        else:
            raise ValueError(f'Generator "{generator_name}" is not supported.')

    def _load_configuration_for_training(self, training_configuration_override: Optional[dict]):
        ...
        # if training config is not inside the experiment dir, load the override one or the project one
        if self.experiment.training_config is None:
            training_config = training_configuration_override or basic_file_utils.load_yaml(_TRAINING_CONFIG_PATH)
